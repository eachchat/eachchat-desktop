...
function MatrixCall(opts) {
  ...
  this.URL = opts.URL; // Array of Objects with urls, username, credential keys
  ...
  this.didConnect = false; // A queue for candidates waiting to go out.
  // We try to amalgamate candidates into a single candidate message where
  // possible

  this.candidateSendQueue = [];
  this.candidateSendTries = 0; // Lookup from opaque queue ID to a promise for media element operations that
...
utils.inherits(MatrixCall, _events.EventEmitter);
/**
...
MatrixCall.prototype.placeScreenSharingCall = async function (remoteVideoElement, localVideoElement) {
  ...
  try {
    self.screenSharingStream = await this.webRtc.getDisplayMedia({
      'audio': false
    });
...
MatrixCall.prototype.playElement = function (element, queueId) {
  _logger.logger.log("queuing play on " + queueId + " and element " + element); // XXX: FIXME: Does this leak elements, given the old promises
  ...
  if (this.mediaPromises[queueId]) {
    ...
    this.mediaPromises[queueId] = this.mediaPromises[queueId].then(function () {
      _logger.logger.log("previous promise completed for " + queueId);

      return element.play();
    }, function () {
      _logger.logger.log("previous promise failed for " + queueId);
...
MatrixCall.prototype.pauseElement = function (element, queueId) {
  _logger.logger.log("queuing pause on " + queueId + " and element " + element);
...
MatrixCall.prototype.assignElement = function (element, srcObject, queueId) {
  _logger.logger.log("queuing assign on " + queueId + " element " + element + " for " + srcObject);
...
MatrixCall.prototype._initWithInvite = function (event) {
  ...
  this.direction = 'inbound'; // firefox and OpenWebRTC's RTCPeerConnection doesn't add streams until it
...
MatrixCall.prototype.answer = function () {
  ...
  if (!this.localAVStream && !this.waitForLocalAVStream) {
    ...
    this.webRtc.getUserMedia(constraints, hookCallback(self, self._maybeGotUserMediaForAnswer), hookCallback(self, self._maybeGotUserMediaForAnswer));
...
MatrixCall.prototype._maybeGotUserMediaForInvite = function (stream) {
  ...
  if (stream instanceof MediaStream) {
    ...
    this.localAVStream = stream; // why do we enable audio (and only audio) tracks here? -- matthew
...
MatrixCall.prototype._sendAnswer = function (stream) {
  sendEvent(this, 'm.call.answer', this._answerContent).then(() => {
    setState(this, 'connecting'); // If this isn't the first time we've tried to send the answer,
...
MatrixCall.prototype._gotLocalIceCandidate = function (event) {
  if (event.candidate) {
    ...
    if (this.state == 'ended') return; // As with the offer, note we need to make a copy of this object, not
...
MatrixCall.prototype._onIceConnectionStateChanged = function () {
  ...
  debuglog("Ice connection state changed to: " + this.peerConn.iceConnectionState); // ideally we'd consider the call to be connected when we get media but
...
MatrixCall.prototype._onAddStream = function (event) {
  ...
  forAllTracksOnStream(s, function (t) {
    debuglog("Track id " + t.id + " added"); // not currently implemented in chrome
...
const sendCandidate = function (self, content) {
  ...
  self.candidateSendQueue.push(content); // Don't send the ICE candidates yet if the call is in the ringing state: this
...
const _tryPlayRemoteAudioStream = async function (self) {
  if (self.getRemoteAudioElement() && self.remoteAStream) {
    const player = self.getRemoteAudioElement(); // if audioOutput is non-default:
...
const _sendCandidateQueue = function (self) {
  ...
  sendEvent(self, 'm.call.candidates', content).then(function () {
    ...
    if (self.candidateSendTries > 5) {
      debuglog("Failed to send candidates on attempt " + self.candidateSendTries + ". Giving up for now.");
      self.candidateSendTries = 0;
      return;
    }
...
function createNewMatrixCall(client, roomId, options) {
  ...
  const getDisplayMedia = w.navigator.mediaDevices && w.navigator.mediaDevices.getDisplayMedia || w.navigator.getDisplayMedia;
  ...
  try {
    ...
    _logger.logger.error("Failed to set up WebRTC object: possible browser interference?");
