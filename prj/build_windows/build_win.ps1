...
$nsisPath = Join-Path $build "windows_installer.nsi";
$nshPath = Join-Path $build "installer.nsh";
$distPath = Join-Path $build "win-unpacked";
...
echo "Sign"
cd $sign;
./SignBatch.bat $build\win-unpacked;

cd $nsis;
Copy-Item $prj\build_windows\windows_installer.nsi $build\ -Force -Recurse;
